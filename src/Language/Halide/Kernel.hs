...
setArgvStorage
  ...
  -> Arguments args
  -> IO ()
setArgvStorage (ArgvStorage argv scalarStorage) inputs = do
...
-- callableToFunction'
--   :: forall f args
--    . (FunctionArguments f ~ args, All ValidArgument args, KnownNat (Length args), Curry args (IO ()) f)
--   => Callable' f
--   -> IO f
-- callableToFunction' (Callable' callable) = do
--   context <- newEmptyCxxUserContext
--   -- +1 comes from CxxUserContext
--   let argc = 1 + fromIntegral (natVal (Proxy @(Length args)))
...
--         touch context
--         touch callable
--   pure $ curryG @args @(IO ()) kernel
--   where
--     _ = keepRedundantConstraint (Proxy @(FunctionArguments f ~ args))
...
type family LoweredOutputs (t :: Type) :: [Type] where
  LoweredOutputs (Func t n (Expr a1, Expr a2)) = '[Ptr (HalideBuffer n a1), Ptr (HalideBuffer n a2)]
  LoweredOutputs (Func t n (Expr a)) = '[Ptr (HalideBuffer n a)]
...
compileToLoweredStmt
  :: forall n a f. (IsFuncBuilder f n a) => StmtOutputFormat -> Target -> f -> IO Text
