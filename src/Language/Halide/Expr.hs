...
mkVar :: HasCallStack => Text -> IO (Expr Int32)
mkVar name =
  wrapCxxExpr
    =<< [CU.exp| Halide::Expr* {
...
-- | Wrap a raw @Halide::Expr@ pointer in a Haskell value that will
-- automatically destroy the C++ object when the Haskell value is garbage collected.
wrapCxxExpr :: (HasCallStack, IsHalideType a) => Ptr CxxExpr -> IO (Expr a)
wrapCxxExpr = checkType <=< (fmap Expr . newForeignPtr deleteCxxExpr)

checkType :: forall a. (HasCallStack, IsHalideType a) => Expr a -> IO (Expr a)
checkType expr =
  withExpr expr $ \e -> do
    let hsType = halideTypeFor (Proxy @a)
    cxxType <- cxxGetType e
    unless (cxxType == hsType) . error $
      "Type mismatch: C++ Expr has type "
        <> show cxxType
        <> ", but Haskell Expr has type"
        <> show hsType
    pure expr
  where
    cxxGetType e =
